...
public abstract class TransferManager {
    ...
    private boolean networkListener = false;
    ...
    public synchronized void doNext() {
        if (!waitingList.isEmpty() && transferringList.size() < TRANSFER_MAX_COUNT) {
            ...
            if (task instanceof UploadTask) {
                if (((UploadTask) task).getUploadFile().isUpload()) {
                    executeTask(task);
                } else {
                    waitingList.add(task);
                    networkListener();
     ...
     *
     * @return true, if there are tasks whose {@link com.ateneacloud.drive.transfer.TaskState} is {@code TRANSFERRING}.
     * false, otherwise.
     */
    public boolean isTransferring() {
        ...
        for (TransferTaskInfo transferTaskInfo : transferTaskInfos) {
            if (transferTaskInfo.state.equals(TaskState.TRANSFERRING)) return true;
        }
        return false;
    }
    ...
    private void networkListener() {
        ...
        service.execute(() -> {
            if (!networkListener) {
                ...
                boolean oldIsConnect = NetworkUtils.isConnectedToWifi();
                while (!waitingList.isEmpty()) {
                    ...
                    boolean newIsConnect = NetworkUtils.isConnectedToWifi();

                    if (newIsConnect != oldIsConnect) {
                        oldIsConnect = newIsConnect;

                        if (newIsConnect && transferringList.isEmpty() && !waitingList.isEmpty()) {
                            try {
                                doNext();
                            } catch (Exception e) {
                                e.printStackTrace();
