            ...
            public void run() {
                for (AutoUpdateInfo info : infos) {
                    if (info.canLocalDecrypt()) {
                        txService.addTaskToUploadQue(new SeafUploadFile(info.account, info.repoID,
                                info.repoName, info.parentDir, info.localPath, true, true));
                    } else {
                        txService.addUploadTask(new SeafUploadFile(info.account, info.repoID, info.repoName,
