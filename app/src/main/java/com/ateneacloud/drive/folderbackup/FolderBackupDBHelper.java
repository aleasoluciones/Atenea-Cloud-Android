...
public class FolderBackupDBHelper extends SQLiteOpenHelper {
    ...
    private FolderBackupInfo getCursorToBackupUpdateInfo(Cursor c) {
        String repoID = c.getString(1);
        String repoName = c.getString(2);
        String parentDir = c.getString(3);
        String fileName = c.getString(4);
        String filePath = c.getString(5);
        String fileSize = c.getString(6);
        if (!new File(filePath).exists()) {
            filePath = null;
        }
        FolderBackupInfo info = new FolderBackupInfo(repoID, repoName,
    ...
    public List<FolderBackupInfo> getAllFolderBackupInfo() {
        List<FolderBackupInfo> folderBackupInfoList = new ArrayList<>();
        Cursor cursor = database.query(
                SAVE_FOLDER_BACKUP_FILE_TABLE_NAME,  // Nombre de la tabla
                null,  // Columnas (null obtiene todas las columnas)
                null,  // Cláusula WHERE
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    FolderBackupInfo folderBackupInfo = getCursorToBackupUpdateInfo(cursor);
                    folderBackupInfoList.add(folderBackupInfo);
                } while (cursor.moveToNext());
            }
            cursor.close();
        }

        return folderBackupInfoList;
    }
}
