        ...
        for (FileBean fb : fileBeanList) {
            if (fb.isDir()) {
                ...
                if (fileInfo != null && !TextUtils.isEmpty(fileInfo.filePath)) {
                    ...
                    int taskID = txService.addTaskToSourceQue(Utils.TRANSFER_FOLDER_TAG, new SeafUploadFile(currentAccount, repoConfig.getRepoID(), repoConfig.getRepoName(), parentPath, fb.getFilePath(), false, false));
                    if (taskID != 0) {
