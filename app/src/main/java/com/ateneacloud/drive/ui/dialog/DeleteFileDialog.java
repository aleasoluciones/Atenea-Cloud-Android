...
class DeleteTask extends TaskDialog.Task {
        ...
        try {
            ...
            if (dirents != null) {
                for (SeafDirent dirent : dirents) {
                    DeleteCell cell = dirent.isDir() ? new DeleteCell(dirent.id, repoID, path + "/" + dirent.name) : new DeleteCell(repoID, path + "/" + dirent.name, dirent.isDir());
                    manager.addTaskToQue(cell);
                }
                manager.doNext();
            } else
                dataManager.delete(null, repoID, path, isdir);
    ...
    class DeleteTaskManager {
        ...
        public synchronized void doNext() {
            if (!waitingList.isEmpty()) {
                ...
                try {
                    if (cell.isdir) {
                        dataManager.delete(cell.id, cell.getRepoID(), cell.getPath(), cell.isdir);
                    } else {
                        dataManager.delete(null, cell.getRepoID(), cell.getPath(), cell.isdir);
                    }
    ...
    class DeleteCell {
        ...
        public DeleteCell(String id, String repoID, String path) {
            this.repoID = repoID;
            this.path = path;
            this.isdir = true;
        }
